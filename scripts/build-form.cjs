    ...
    switch (field.type) {
      ...
      case 'number':
        ...
        <div class="mb-4">
          ...
          <input type="${field.type}" id="${key}" name="${key}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" ${required}>
        </div>
        `
        break
      case 'textarea':
        ...
        <div class="mb-4">
          <label for="${key}" class="block text-sm font-medium text-gray-700">${label}</label>
          <textarea id="${key}" name="${key}" rows="4" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" ${required}></textarea>
        </div>
        `
        break
      case 'select':
        ...
        <div class="mb-4">
          <label for="${key}" class="block text-sm font-medium text-gray-700">${label}</label>
          <select id="${key}" name="${key}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" ${required}>
            ${optionsHtml}
          </select>
        </div>
        `
      ...
      case 'date':
        ...
        <div class="mb-4">
          <label for="${key}" class="block text-sm font-medium text-gray-700">${label}</label>
          <input type="date" id="${key}" name="${key}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" ${required}>
        </div>
        `
        break
      default:
