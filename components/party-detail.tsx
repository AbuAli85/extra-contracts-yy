...
export default function PartyDetail({ partyId }: PartyDetailProps) {
  ...
  useEffect(() => {
    ...
    supabase.from('party_notes' as any).select('*').eq('party_id', partyId).then(({ data }) => setNotes(data || []))
    supabase.from('party_tags' as any).select('*').eq('party_id', partyId).then(({ data }) => setTags(data || []))
    supabase.from('party_activities' as any).select('*').eq('party_id', partyId).then(({ data }) => setActivities(data || []))
    supabase.from('parties').select('owner_id').eq('id', partyId).single().then(({ data }) => setOwnerId(data?.owner_id || null))
    supabase.from('profiles' as any).select('id, full_name').then(({ data }) => setUsers(data || []))
    supabase.from('party_files' as any).select('*').eq('party_id', partyId).then(({ data }) => setFiles(data || []))
  }, [partyId])

  const addNote = async () => {
    if (!noteInput) return
    await supabase.from('party_notes' as any).insert({ party_id: partyId, user_id: user.id, note: noteInput })
    setNoteInput('')
    supabase.from('party_notes' as any).select('*').eq('party_id', partyId).then(({ data }) => setNotes(data || []))
  }
  ...
  const addTag = async () => {
    if (!tagInput) return
    await supabase.from('party_tags' as any).insert({ party_id: partyId, tag: tagInput })
    setTagInput('')
    supabase.from('party_tags' as any).select('*').eq('party_id', partyId).then(({ data }) => setTags(data || []))
  }
  ...
  const uploadFile = async (e) => {
    ...
    await supabase.from('party_files' as any).insert({
      party_id: partyId,
      file_name: file.name,
      file_url: fileUrl
    })
  ...
  const deleteFile = async (file) => {
    ...
    if (
      ...
      // Remove from DB
      await supabase.from('party_files' as any).delete().eq('id', file.id)
      supabase.from('party_files' as any).select('*').eq('party_id', partyId).then(({ data }) => setFiles(data || []))
    } else {
